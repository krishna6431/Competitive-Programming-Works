...
struct cmp
{
    bool operator()(const pair<ll, pair<ll, ll>> &a, const pair<ll, pair<ll, ll>> &b)
    {
        ...
        if (lenA != lenB)
        {
            return lenA < lenB; // Return the larger length first.
        }

        return a.second.first > b.second.first; // If lengths are same, return the one with the smaller starting index.
    }
};
...
void HarHarMahadev()
{
    ...
    for (ll i = 1; i <= n; i++)
    {
        auto [len, subarray] = pq.top();
        pq.pop();
        ll l = subarray.first;
        ll r = subarray.second;
        ...
        if (pos - l > 0)
            pq.push({pos - l, {l, pos - 1}});
        if (r - pos > 0)
            pq.push({r - pos, {pos + 1, r}});
    }
