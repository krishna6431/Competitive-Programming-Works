...
int merge(vector<ll> &arr, int left, int mid, int right)
{
    int len1 = mid - left + 1;
    int len2 = right - mid;
    ll temp[len1 + len2];
    ...
    {
        int temp = (lower_bound(arr.begin() + left, arr.begin() + left + len1, arr[mid + 1 + i] + 1) - (arr.begin() + left));
        cout << temp << endl;
        count += len1 - temp;
    }
    ...
    while (i < len1 or j < len2)
    {
        ...
        else if (arr[left + i] <= arr[mid + 1 + j])
        {
            temp[k++] = arr[left + i];
            i++;
        }
...
int mergesort(vector<ll> &arr, int left, int right)
{
    if (left == right)
        return 0;
    ...
    int c3 = merge(arr, left, mid, right);
    return c1 + c2 + c3;
}
