...
void HarHarMahadev()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            hcuts.insert(cut);
            auto itr = hcuts.lower_bound(cut);
            auto it1 = itr;
            auto it2 = itr;
            it1--;
            it2++;
            height.erase(height.find(*it2 - *it1));
            height.insert(*it2 - *itr);
            height.insert(*itr - *it1);
        }
        else
        {
            vcuts.insert(cut);
            auto itr = vcuts.lower_bound(cut);
            auto it1 = itr;
            auto it2 = itr;
            it1--;
            it2++;
            width.erase(width.find(*it2 - *it1));
            width.insert(*it2 - *itr);
            width.insert(*itr - *it1);
        }
        cout << (*prev(height.end())) * (*prev(width.end())) << endl;
    }
}
