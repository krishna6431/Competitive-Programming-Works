...
void HarHarMahadev()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= m; j++)
        {
            ll a = mod_add(dp[i - 1][j], dp[i][j - 1], mod);
            ll b = mod_sub(a, dp[i - 1][j - 1], mod);
            ll c = mod_add(b, mat[i][j], mod);
            dp[i][j] = c;
        }
    }
    while (q--)
    {
        int x1, y1, x2, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        ll a = mod_sub(dp[x2][y2], dp[x1 - 1][y2], mod);
        ll b = mod_sub(a, dp[x2][y1 - 1], mod);
        ll c = mod_add(b, dp[x1 - 1][y1 - 1], mod);
        cout << c << endl;
    }
}
