...
void HarHarMahadev()
{
    ll n;
    cin >> n;
    ll diagonal = mod_mul(n, n - 1, mod);
    diagonal = mod_mul(diagonal, mod_inv_prime(2, mod), mod);
    ll x = mod_sub(diagonal, n, mod);
    ll intersect = mod_mul(n, mod_mul(n - 1, mod_mul(n - 2, n - 3, mod), mod), mod);
    intersect = mod_mul(intersect, mod_inv_prime(24, mod), mod);
    cout << mod_add(x, intersect, mod) + 1 << endl;
}
