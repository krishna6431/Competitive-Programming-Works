...
void HarHarMahadev()
{
    ...
    dist.clear();
    graph.clear();
    graph.resize(n + 1, vll(m + 1));
    dist.resize(n + 1, vll(m + 1, 1e9));
    for (int i = 1; i <= n; i++)
    {
        for (int j = 1; j <= m; j++)
        {
    ...
    while (!dq.empty())
    {
        ...
        {
            ll newx = curr.fr + dx[dir];
            ll newy = curr.sc + dy[dir];
            if (isValid(newx, newy))
            {
                ...
                {
                    // bug("Right");
                    if (graph[curr.fr][curr.sc] == 4)
                    {
                        // bug("nkn");
                        if (dist[newx][newy] > dist[curr.fr][curr.sc])
                        {
                            dist[newx][newy] = dist[curr.fr][curr.sc];
                    ...
                    {
                        if (dist[newx][newy] > dist[curr.fr][curr.sc] + 1)
                        {
                            dist[newx][newy] = dist[curr.fr][curr.sc] + 1;
                            dq.push_back({newx, newy});
                        }
                    }
                }
                else if (newx == curr.fr - 1)
                {
                    // bug("left");
                    if (graph[curr.fr][curr.sc] == 3)
                    {
                ...
                else if (newy == curr.sc - 1)
                {
                    // bug("up");

                    if (graph[curr.fr][curr.sc] == 2)
                    {
                        if (dist[newx][newy] > dist[curr.fr][curr.sc])
                        {
                            dist[newx][newy] = dist[curr.fr][curr.sc];
