...
// CODE WRITTEN BY mr_krishna(cc,cf,google)/krishna_6431(gfg,leet)4
vvll graph;
vvll dist;
ll n, m;
ll dx[4] = {0, 1, 0, -1};
...
void HarHarMahadev()
{
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < sz; i++)
        {
            ...
            for (int k = 0; k < 4; k++)
            {
                ll newx = curr.fr + dx[k];
                ll newy = curr.sc + dy[k];
                if (isValid(newx, newy))
                {
                    if (graph[newx][newy] == 1 and 1 + dist[curr.fr][curr.sc] < dist[newx][newy])
                    {
                        q.push({newx, newy});
