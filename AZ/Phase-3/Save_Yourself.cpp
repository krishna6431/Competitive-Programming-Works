...
void HarHarMahadev()
{
    ...
    dist.resize(n, vll(m, 1e9));
    distMan.resize(n, vll(m, 1e9));
    queue<pll> q;
    queue<pll> manq;
    for (int i = 0; i < n; i++)
    {
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < sz; i++)
        {
            ...
            {
                ll newx = curr.first + dx[dir];
                ll newy = curr.second + dy[dir];
                if (isValid(newx, newy, n, m) && graph[newx][newy] == 1 and dist[newx][newy] > 1 + dist[curr.first][curr.second])
                {
                    q.push({newx, newy});
                    dist[newx][newy] = 1 + dist[curr.first][curr.second];
                }
            }
        }
    }
    ...
    while (!manq.empty())
    {
        ...
        for (int i = 0; i < sz; i++)
        {
            auto curr = manq.front();
            manq.pop();
            for (int dir = 0; dir < 4; dir++)
            {
                ll newx = curr.first + dx[dir];
                ll newy = curr.second + dy[dir];
                if (isValid(newx, newy, n, m) && graph[newx][newy] == 1 and distMan[newx][newy] > 1 + distMan[curr.first][curr.second])
                {
                    manq.push({newx, newy});
                    distMan[newx][newy] = 1 + distMan[curr.first][curr.second];
                }
            }
        }
    }
    ...
    {
        if (dist[0][col] > distMan[0][col])
        {
            shortestPath = min(shortestPath, distMan[0][col]);
        }
        if (dist[n - 1][col] > distMan[n - 1][col])
        {
            shortestPath = min(shortestPath, distMan[n - 1][col]);
        }
    }
    for (int i = 0; i < n; i++)
    {
        if (dist[i][0] > distMan[i][0])
        {
            shortestPath = min(shortestPath, distMan[i][0]);
        }
        if (dist[i][m - 1] > distMan[i][m - 1])
        {
            shortestPath = min(shortestPath, distMan[i][m - 1]);
        }
    }
    if (shortestPath != 1e9)
    {
