...
void HarHarMahadev()
{
    ...
    while (true)
    {
        if (ll(lower_bound(v.begin() + temp + 1, v.end(), prefixCount + 1) - v.begin()) >= n)
            break;
        else
            temp = ll(lower_bound(v.begin() + temp + 1, v.end(), prefixCount + 1) - v.begin());
        prefixCount += v[temp];
        prefix[temp] = 1;
    }
    ll sum = accumulate(all(prefix), 0ll);
    cout << n - sum << endl;
    set<ll> st;
    set<ll> nst;
    ll index = 0;
