...
void HarHarMahadev()
{
    ...
    // dp[i][j] = edit distance between str1 and str2 with str1 with first i char and str2j with first j char
    // base case
    dp[0][0] = 0;
    for (int i = 1; i <= n; i++)
    {
        dp[i][0] = i;
        // str2 exhausted and str1 has still i charcters
    }
    for (int j = 1; j <= m; j++)
    {
        dp[0][j] = j;
        // str1 exhausted and str2 has still j charcters
    }
