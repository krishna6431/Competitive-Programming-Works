...
void HarHarMahadev()
{
    ...
    vvll prefix(n + 1, vll(4, 0));
    for (int i = 1; i <= n; i++)
    {
        prefix[i] = prefix[i - 1];
        prefix[i][v[i]] = prefix[i - 1][v[i]] + 1;
    }
    ...
    for (int i = 0; i < q; i++)
    {
        ll l, r;
        cin >> l >> r;
        ll oneCount = prefix[r][1] - prefix[l - 1][1];
        ll twoCount = prefix[r][2] - prefix[l - 1][2];
        ll threeCount = prefix[r][3] - prefix[l - 1][3];
        if ((r - l + 1) & 1)
        {
            cout << "No" << endl;
            continue;
        ...
        ll num = (r - l + 1) / 2;
        ll isOk = 0;
        isOk += oneCount >= num ? 1 : 0;
        isOk += twoCount >= num ? 1 : 0;
        isOk += threeCount >= num ? 1 : 0;
        if (oneCount <= num && twoCount <= num && threeCount <= num)
        {
            cout << "Yes" << endl;
        }
        else
