...
void HarHarMahadev()
{
    ...
    sort(all(v));
    ll ans = 1e18;
    ans = min(ans, abs(v[0] - v[n - 1]));
    ans = min(ans, abs(v[0] - v[n - 3]));
    ans = min(ans, abs(v[2] - v[n - 1]));
    ans = min(ans, abs(v[0] - v[n - 2]));
    ans = min(ans, abs(v[1] - v[n - 1]));
    ans = min(ans, abs(v[1] - v[n - 2]));
    cout << ans << endl;
}
