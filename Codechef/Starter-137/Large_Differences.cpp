...
void HarHarMahadev()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        ll prev = abs(v[i] - v[i - 1]) + abs(v[i] - v[i + 1]);
        ll mini = abs(1ll - v[i - 1]) + abs(1ll - v[i + 1]);
        ll maxi = abs(k - v[i - 1]) + abs(k - v[i + 1]);
        ans = max({ans, tot - prev + mini, tot - prev + maxi});
    }
    cout << ans << endl;
}
