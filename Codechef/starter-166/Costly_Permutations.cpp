...
void dfs(ll node, const vector<ll> &graph, vector<bool> &visited, vector<bool> &stack, vector<ll> &cmin)
{
    visited[node] = true;
    stack[node] = true;
    ...
    if (!visited[next])
    {
        dfs(next, graph, visited, stack, cmin);
    }
    else if (stack[next])
    {
        ll len = 0;
...
vll findCycles(vll &graph)
{
    ...
    for (int i = 1; i < n; ++i)
    {
        if (!visited[i])
        {
            dfs(i, graph, visited, stack, cmin);
        }
    }
...
void HarHarMahadev()
{
    ...
    v.insert(v.begin() + 0, 0ll);
    // for (auto x : v)
    // {
    //     cout << x << " ";
    // }
    // cout << endl;
    vll lens = findCycles(v);
