...
void HarHarMahadev()
{
    ...
    map<ll, pair<ll, ll>> st;
    for (int i = 0; i < k; i++)
    {
        st[i] = {vp[i].first, vp[i].second};
    }
    ...
    else if (st.find(a) == st.end() and st.find(b) == st.end())
    {
        auto it1 = vp[a - 1];
        auto it2 = vp[b - 1];
        cout << abs(it2.second - it1.second) + abs(it2.first - it1.first) << endl;
    }
    else
    {
    }
