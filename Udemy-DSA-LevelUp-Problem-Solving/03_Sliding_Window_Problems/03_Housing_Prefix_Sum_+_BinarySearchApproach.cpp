...
//prefix sum approach
vector<pair<int, int>> prefixSum_BinarySearch_Housing(vector<int> v, int k)
{
    ...
    {
        int findPotentialElement = prefix[i] + k;
        if (binary_search(prefix.begin() + i, prefix.end(), findPotentialElement))
        {
            int loc_j = lower_bound(prefix.begin() + i, prefix.end(), findPotentialElement) - prefix.begin();
            ans.push_back({i + 1, loc_j});
        }
    }
    return ans;
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<pair<int, int>> ans = prefixSum_BinarySearch_Housing(input, k);
        for (auto x : ans)
        {
