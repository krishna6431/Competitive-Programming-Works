...
void HarHarMahadev()
{
    ...
    for (int len = 1; len <= n; len++)
    {
        ...
        {
            int j = i + len - 1;
            // Calculate the maximum score difference for the current player
            // 0 indicates the first player's turn, 1 indicates the second player's turn.
            ll pickLeft = arr[i] + dp[i + 1][j][1];
            ll pickRight = arr[j] + dp[i][j - 1][1];
