...
void HarHarMahadev()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ...
        if (mp.count(temp))
        {
            mp[temp].first = min(mp[temp].first, i);
            mp[temp].second = max(mp[temp].second, i);
        }
        else
        {
            mp[temp].first = i;
    ...
    for (auto x : mp)
    {
        ...
        vp.pb({x.second.first, x.second.second});
    }
    ...
    {
        curmax = vp[0].second - vp[0].first + 1;
    }
    else
    {
        curmax = 0;
    ...
    for (int i = 1; i < vp.size(); i++)
    {
        ...
        {
            curmax = max(curmax, vp[i].second - vp[i].first);
        }
        else
        {
            if (i == vp.size() - 1)
                flag = true;
            ans += curmax;
            if (vp[i].second != vp[i].first)
            {
                curmax = vp[i].second - vp[i].first + 1;
            }
            else
            {
                curmax = 0;
