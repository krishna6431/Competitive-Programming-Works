...
void doSolve(vll &memo1, vll &memo2, vll &weight1, vll &weight2, ll i, vll &v)
{
    if (memo1[i] == memo2[i - 1] + v[i])
    {
        weight1[i] = (weight1[i] + weight2[i - 1]) % mod;
    }
    if (memo1[i] == abs(memo2[i - 1] + v[i]))
    {
        weight1[i] = (weight1[i] + weight2[i - 1]) % mod;
    }
}

void doSolve2(vll &memo1, vll &memo2, vll &weight1, vll &weight2, ll i, vll &v)
{
    if (memo2[i] == memo2[i - 1] + v[i])
    {
        weight2[i] = (weight2[i] + weight2[i - 1]) % mod;
    }
    if (memo2[i] == abs(memo2[i - 1] + v[i]))
    {
        weight2[i] = (weight2[i] + weight2[i - 1]) % mod;
    }
}
...
void HarHarMahadev()
{
    ...
    vector<ll> back1(n + 1, 0), back2(n + 1, 0);

    dp1[1] = max(v[1], abs(v[1]));
    dp2[1] = min(v[1], abs(v[1]));
    v[1] < 0 ? back1[1] = back2[1] = 1 : back1[1] = back2[1] = 2;

    for (int i = 2; i <= n; ++i)
    {
        ll a = dp1[i - 1] + v[i];
        ll b = abs(dp1[i - 1] + v[i]);
        ll c = dp2[i - 1] + v[i];
        ll d = abs(dp2[i - 1] + v[i]);
        dp1[i] = max3(a, b, max(c, d));

        ll sumVal = (back1[i] + back1[i - 1]);
        ll checker3 = dp1[i - 1] + v[i];
        if (dp1[i] == checker3)
        {
            back1[i] = sumVal % mod;
        }

        sumVal = (back1[i] + back1[i - 1]);
        ll checker4 = abs(dp1[i - 1] + v[i]);
        if (dp1[i] == checker4)
        {
            back1[i] = sumVal % mod;
        }
        ...
        if (dp1[i - 1] != dp2[i - 1])
        {
            doSolve(dp1, dp2, back1, back2, i, v);
        }

        dp2[i] = min3(a, b, min(c, d));
        sumVal = (back2[i] + back1[i - 1]);
        ll checker2 = dp1[i - 1] + v[i];
        if (dp2[i] == checker2)
        {
            back2[i] = sumVal % mod;
        }
        sumVal = (back2[i] + back1[i - 1]);
        ll checker = abs(dp1[i - 1] + v[i]);
        if (dp2[i] == checker)
        {
            back2[i] = sumVal % mod;
        }
        ...
        if (dp1[i - 1] != dp2[i - 1])
        {
            doSolve2(dp1, dp2, back1, back2, i, v);
        }
    }
    ll ans = back1[n];
    cout << ans << endl;
