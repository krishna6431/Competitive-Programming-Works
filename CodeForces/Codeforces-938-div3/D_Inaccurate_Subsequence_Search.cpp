...
void doSolve(map<ll, ll> &mp1, map<ll, ll> &mp2, ll &temp, ll &i, ll &m, vll &a)
{
    ll mini = min(mp1[a[i]], mp2[a[i]]);
    ll mini1 = min(mp1[a[i + m]], mp2[a[i + m]]);
    temp -= mini;
    temp -= mini1;
    mp2[a[i]]--;
    mp2[a[i + m]]++;
