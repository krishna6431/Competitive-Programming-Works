...
ll dfs(ll node, vector<ll> &visited, vector<ll> adj[], vector<ll> &leaf)
{
    visited[node] = 1;
    ll ans = 0;
    ll flag = false;
    for (auto x : adj[node])
    {
        if (!visited[x])
        {
            ans += dfs(x, visited, adj, leaf);
            flag = true;
        }
    }
    // bug(ans);
    if (flag)
        return leaf[node] = ans;
    return leaf[node] = 1;
}
void HarHarMahadev()
{
    ...
    vector<ll> vis(n + 1, 0);
    vector<ll> leafCount(n + 1, 0);
    leafCount[1] = dfs(1, vis, adj, leafCount);
    ll assum;
    cin >> assum;
    while (assum--)
    {
