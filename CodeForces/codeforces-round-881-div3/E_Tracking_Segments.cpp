...
bool predicate(ll mid, vpll &vp, vll &q, ll n)
{
    vll ans(n + 1, 0);
    for (int i = 0; i < mid; i++)
    {
    ...
    // debug(prefix);
    // cout << "sz: " << vp.size() << endl;
    // debug(vp);
    for (int i = 0; i < vp.size(); i++)
    {
        ll zeroCount = prefix[vp[i].second].first - prefix[vp[i].first - 1].first;
        ll oneCount = prefix[vp[i].second].second - prefix[vp[i].first - 1].second;
...
void HarHarMahadev()
{
    ...
    ll start = 1, end = query.size();
    ll ans = -1;
    while (start <= end)
    {
        ll mid = start + (end - start) / 2;
        if (predicate(mid, vp, query, n))
        {
            end = mid - 1;
            ans = mid;
        }
