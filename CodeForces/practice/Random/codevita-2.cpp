...
ll dp[1001][1001];
ll solve(vvll &v, ll a, ll b, ll x, ll y)
{
    if (a < 0 or a >= (ll)v.size() or b < 0 or b >= (ll)v[0].size())
    {
        return 1e9;
    }
    if (dp[a][b] != -1)
    ...
    ll right = v[a][b] + solve(v, a, b + 1, x, y);
    ll down = v[a][b] + solve(v, a + 1, b, x, y);

    return dp[a][b] = min(right, down);
}
void HarHarMahadev()
{
    ll n, m;
