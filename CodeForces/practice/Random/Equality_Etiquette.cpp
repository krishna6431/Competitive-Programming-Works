...
void HarHarMahadev()
{
    ...
    ll newb = (b + 1) / 2;
    ll h, l;
    h = min(1ll + max({newa - 1, newb - 1, a - newa, b - newb}), a);
    l = max(1ll, a - max({newa - 1, newb - 1, a - newa, b - newb}));
    ll hh, lll;
    hh = min(1ll + max({newa - 1, newb - 1, a - newa, b - newb}), b);
    lll = max(1ll, b - max({newa - 1, newb - 1, a - newa, b - newb}));
    l--;
    lll--;
    ll ans1 = h * (h + 1) / 2 + l * (l + 1) / 2;
    ll ans2 = hh * (hh + 1) / 2 + lll * (lll + 1) / 2;
    ll ans = ans1 * ans2;
    cout << ans << endl;
}
int main()
