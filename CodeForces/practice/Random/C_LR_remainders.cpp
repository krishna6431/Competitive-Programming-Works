...
// CODE WRITTEN BY mr_krishna(cc,cf,google)/krishna_6431(gfg,leet)4vector<int> prefix_mod_product(const vector<int>& arr, int modulus) {
class segment_tree
{
private:
    ll modd;
...
public:
    ...
    void build(vector<ll> &arr, ll start, ll end, ll index)
    {
        ...
        seg_tree[index] = (seg_tree[2 * index] * seg_tree[2 * index + 1]) % modd;
        return;
    }

    int query(ll start, ll end, ll qStart, ll qEnd, ll index)
    {
        // complate overlap
        if (start >= qStart and end <= qEnd)
        {
            return seg_tree[index] % modd;
        }
        // no overlap
...
void HarHarMahadev()
{
    ...
    string q;
    cin >> q;
    segment_tree st(5 * n + 5, k);
    st.build(arr, 0, n - 1, 1);
    ll left = 0, right = n - 1;
    for (auto x : q)
    {
        cout << st.query(0, n - 1, left, right, 1) << " ";
        if (x == 'R')
        {
            right--;
        }
