...
void solveMe(ll r = 0)
{
    ...
    for (auto i : graph[r])
    {
        {
            solveMe(i);
            prefixa[r] += min(prefixb[i] + 1, prefixa[i]);
            prefixb[r] += min(prefixa[i] + 1, prefixb[i]);
        }
    }
    pattern[r] == 'S' ? prefixa[r] = 1e6 : prefixb[r] = 1e6;
}
void HarHarMahadev()
{
    ll n, k;
