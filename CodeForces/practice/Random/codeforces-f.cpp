...
ll pre = 0;
ll helper(vector<ll> &v, ll n, ll idx, ll k, vector<ll> &dp, map<pair<ll, ll>, ll> &mp)
{
    if (k == 0)
    {
        ll ans = 0;
        vector<pair<ll, ll>> temp;
        for (int i = 0; i < (ll)dp.size(); i++)
        {
            if (dp[i])
            {
                ans += dp[i];
    ...
    if (k - v[idx] >= 0)
    {
        // cout<<v[idx]<<" ";
        dp[v[idx]]++;
        op1 = helper(v, n, idx, k - v[idx], dp, mp);
        dp[v[idx]]--;
    }

    ll op2 = helper(v, n, idx + 1, k, dp, mp);
    return mp[{idx, k}] = max(op1, op2);
}
int main()
{
    ...
    // cout << ans << endl;
    for (int i = 0; i < (ll) final.size() - 1; i++)
    {
        cout << final[i] << " ";
    }
    cout << final[(ll) final.size() - 1];
    // cout << endl;
    return 0;
}