...
void HarHarMahadev()
{
    ...
    for (auto &x : mp)
    {
        if (x.first == t)
        {
            textra += x.second.size();
        }
        else
        {
            extra += x.second.size() % 2;
        }
    }
    if (textra >= extra and (textra - extra) % 2 == 0)
    {
        for (auto &x : mp)
        {
            sort(all(x.second));
        ...
        for (auto &x : mp)
        {
            ...
            {
                cout << x.second[i] << x.first << " " << x.second[i + 1] << x.first << endl;
            }
            if (x.second.size() & 1)
            {
                string str = "";
                str += to_string(x.second.back());
                str += x.first;
                rem.pb(str);
            }
        }
        ...
        ll left = 0, right = 0;
        sort(all(tr));
        while (left < rem.size() && right < tr.size())
        {
            string a = rem[left];
            string b = tr[right];
            cout << a << " " << b << endl;
