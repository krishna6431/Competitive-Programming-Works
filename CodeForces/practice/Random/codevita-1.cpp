...
void HarHarMahadev()
{
    ...
    for (int i = 0; i < ans[n - 2].size(); i += 2)
    {
        vp.push_back({ans[n - 2][i], i});
    }
    // cout << ans[n - 1] << endl;
    string last = ans[n - 1];
    ll i = 0;
    ...
    while (i < last.size())
    {
        ...
        else if (i + 1 < last.size())
        {
            cout << last[i] << last[i + 1];
            vp[j].sc = (last[i] - '0') * 10 + (last[i + 1] - '0');
            i += 3;
            j++;
        }
        else
    ...
    {
        // case1
        if (start + 3 < n and ans[start][0] == 'i' and ans[start + 3][0] == 's')
        {
            cout << mp[ans[start + 2][ans[start + 2].size() - 1]] << endl;
        }
        // case2
        if (start + 5 < n and ans[start][0] == 'i' and ans[start + 5][0] == 's')
        {
            char a = ans[start + 2][ans[start + 2].size() - 1];
            char b = ans[start + 2][ans[start + 2].size() - 1]
        }
        // case3
        if (ans[start][0] == 'i' and ans[start + 7][0] == 's' and ans[start + 14][0] == 'a')
        {
        }
    }
    printing_debugs(vp);
