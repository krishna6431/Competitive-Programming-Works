...
public:
    ...
    ll query(ll start, ll end, ll qStart, ll qEnd, ll index)
    {
        ...
        ll left = query(start, mid, qStart, qEnd, 2 * index);
        ll right = query(mid + 1, end, qStart, qEnd, 2 * index + 1);
        return (left & right);
    }
...
void HarHarMahadev()
{
    ...
    while (q--)
    {
        ...
        {
            ll mid = start + (end - start) / 2;
            ll tempAns = st.query(0, n - 1, left, mid, 1);
            // cout << "mid: " << mid << " qRes: " << Result << endl;
            if (tempAns >= target)
            {
                start = mid;
                ans = mid;
