...
bool checkPredicate(ll mid, vll &v, ll k)
{
    vll arr;
    arr.pb(mid);
    for (auto x : v)
    ...
    for (int i = 1; i < n; i++)
    {
        ...
        prefix[i] = min(mid, prefix[i]);
    }
    suffix[n - 1] = arr[n - 1];
    for (int i = n - 2; i >= 0; i--)
    {
        suffix[i] = max(suffix[i + 1], arr[i]);
        suffix[i] = min(suffix[i], mid);
    }
    ll water = 0;
    for (int i = 0; i < n; i++)
    {
        water += max(0ll, min(prefix[i], suffix[i]) - arr[i]);
    }
    // cout << "Water Trapped: " << mid << " " << water << endl;
    return water <= k;
}
