...
public:
    ...
    void build(vector<int> &arr, int start, int end, int index, string &str)
    {
        if (start == end)
        {
        ...
        seg_tree[index] = str[index] == '1' ? (seg_tree[2 * index] | seg_tree[2 * index + 1]) : (seg_tree[2 * index] ^ seg_tree[2 * index + 1]);
    }
