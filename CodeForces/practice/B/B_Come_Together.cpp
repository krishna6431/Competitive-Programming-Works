...
void HarHarMahadev()
{
    ...
    ll ans = 0;
    // bug(dir_x_b, dir_x_c);
    if (dir_x_b >= 0 and dir_x_c >= 0)
    {
        ans += min(abs(dir_x_b), abs(dir_x_c));
    }
    else if (dir_x_b < 0 and dir_x_c < 0)
    {
        ans += min(abs(dir_x_b), abs(dir_x_c));
    }

    // bug(dir_y_b, dir_y_c);
    if (dir_y_b >= 0 and dir_y_c >= 0)
    {
        ans += min(abs(dir_y_b), abs(dir_y_c));
    }
    else if (dir_y_b < 0 and dir_y_c < 0)
    {
        ans += min(abs(dir_y_b), abs(dir_y_c));
    }
