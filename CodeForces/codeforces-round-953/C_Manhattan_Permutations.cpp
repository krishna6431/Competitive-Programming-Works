...
void HarHarMahadev()
{
    ...
    {
        while (difference > 0 && i < difference)
        {
            ll t = (abs(ans[i] - (difference + 1)) + abs(ans[difference] - (i + 1)));
            if (m < t)
            {
                difference--;
            }
        ...
        ll tempAns = (abs(ans[i] - (difference + 1)) + abs(ans[difference] - (i + 1)));
        if (check(m, i, difference))
        {
            break;
        }
        m -= tempAns;
        ll temp = ans[i];
        ans[i] = ans[difference];
        ans[difference] = temp;
        difference--;
        if (check(m, i, difference))
        {
