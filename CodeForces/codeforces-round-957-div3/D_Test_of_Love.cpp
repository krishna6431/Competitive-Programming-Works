...
ll dp[200008];
bool solve(ll n, ll m, ll k, string &str)
{
    dp[0] = 0;

    for (ll j = 1; j <= m && j <= n + 1; ++j)
    {
        if (j == n + 1 || str[j - 1] != 'C')
        {
            dp[j] = min(dp[j], dp[0]);
    ...
    for (ll i = 1; i <= n; ++i)
    {
        ...
        if (str[i - 1] == 'L')
        {
            for (ll j = 1; j <= m && i + j <= n + 1; ++j)
            {
                if (i + j == n + 1 || str[i + j - 1] != 'C')
                {
        ...
        if (i > 0 && str[i - 1] == 'W' && dp[i] < k)
        {
            if (i + 1 <= n + 1 && (i + 1 == n + 1 || str[i] != 'C'))
            {
