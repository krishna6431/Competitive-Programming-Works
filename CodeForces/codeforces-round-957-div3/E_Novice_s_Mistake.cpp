...
bool check(ll a, ll b, ll n, string &str)
{
    ll res = n * a - b;
    if (str.size() == 0)
    {
...
void HarHarMahadev()
{
    ...
    cin >> n;
    vpll ans;
    ll abc = to_string(n).size();
    for (ll i = 1; i <= 10000; i++)
    {
        for (ll j = min(10000ll, i * abc); j >= max(1ll, i * abc - 8); j--)
        {

            ll rem = i * abc - j;
            string str = "";
            while (str.size() + abc <= min(rem + abc, 8ll))
            {
                str += to_string(n);
            }
            while (str.size() > rem)
