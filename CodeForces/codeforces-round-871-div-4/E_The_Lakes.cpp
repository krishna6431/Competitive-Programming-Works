...
ll dy[] = {0, +1, 0, -1};
ll ans = 0;
void dfs(int i, int j, vvll &visited, vvll &graph, ll &currsum)
{
    if (i < 0 or i >= graph.size() or j < 0 or j >= graph[0].size() or graph[i][j] == 0 or visited[i][j])
    {
        return;
    }
    currsum += graph[i][j];
    ...
    for (int k = 0; k < 4; k++)
    {
        ll newx = i + dx[k];
        ll newy = j + dy[k];
        dfs(newx, newy, visited, graph, currsum);
    }
}
void HarHarMahadev()
{
