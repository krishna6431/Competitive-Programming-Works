...
int main()
{
    ...
    for (int i = 3; i < 2024; i++)
    {
        ...
        for (int j = 0; j < i; j++)
        {
            ...
            if (j == 0)
            {
                dp[i][j] = (counter * counter) + dp[i - 1][j];
            }
            else if (j == i - 1)
            {
                dp[i][j] = (counter * counter) + dp[i - 1][j - 1];
            }
            else
            {
                dp[i][j] = (counter * counter) + dp[i - 1][j - 1] + dp[i - 1][j];
            }

            if (j - 1 >= 0)
            {
                dp[i][j] = dp[i][j] - dp[subtractPart][j - 1];
            }
            ans[counter] = dp[i][j];
            counter++;
        }
