...
void HarHarMahadev()
{
    ...
    for (int i = n - 1; i >= 0; i--)
    {
        auto itr = lower_bound(prefix.begin(), prefix.begin() + n + 1, prefix[i] + k);
        if (itr == prefix.begin() + n + 1)
        {
            dp[i] += (n - i);
            ans += (n - i);
            continue;
        }
        if (prefix[i] == prefix[itr - prefix.begin()] - k)
        {
            dp[i] += (itr - prefix.begin() - i) + dp[itr - prefix.begin() + 1];
            ans += (itr - prefix.begin() - i) + dp[itr - prefix.begin() + 1];
        }
        else
        {
            dp[i] += (itr - prefix.begin() - i - 1) + dp[itr - prefix.begin()];
            ans += (itr - prefix.begin() - i - 1) + dp[itr - prefix.begin()];
        }
    }
    cout << ans << endl;
}
