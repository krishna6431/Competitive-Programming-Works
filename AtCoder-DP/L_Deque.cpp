...
ll dp[3001][3001][2];
ll solve(ll i, ll j, vll &v, ll turn)
{
    if (i > j)
    {
        return 0;
    ...
    if (turn == 0)
    {
        ll op1 = v[i] + solve(i + 1, j, v, 1 - turn);
        ll op2 = v[j] + solve(i, j - 1, v, 1 - turn);
        ans = max(op1, op2);
    }
    else
    {
        ll op3 = solve(i + 1, j, v, 1 - turn);
        ll op4 = solve(i, j - 1, v, 1 - turn);
        ans = min(op3, op4);
    }
