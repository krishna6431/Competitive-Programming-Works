...
double dist(vector<int> &arr, double x)
{
    double total = 0;
    for (int num : arr)
    {
...
double ts(vector<int> &arr, double left, double right)
{
    ...
    while (right - left > eps)
    {
        ...
        double dm1 = dist(arr, mid1);
        double dm2 = dist(arr, mid2);
...
public:
    long long minimumCost(vector<int> &nums)
    {
        ...
        double left = *min_element(array.begin(), array.end());
        double right = *max_element(array.begin(), array.end());
        double nearest = ts(array, left, right);
        ll l = left;
        ll r = right;
        bool flag = false;
        if (palindrome(nearest))
        {
            flag = true;
        }

        for (ll i = nearest - 1; i >= 0; i--)
        {
            if (palindrome(i))
            {
                l = i;
