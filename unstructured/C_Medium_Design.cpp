...
struct Event
{
    int pos, delta; // delta: +1 for start of segment, -1 for end of segment
    Event(int _pos, int _delta) : pos(_pos), delta(_delta) {}
};
...
void processTestCase(ll n, ll m)
{
    vector<Event> events;
    ...
    for (int i = 1; i <= n; ++i)
    {
        ll l, r;
        cin >> l >> r;
        events.emplace_back(l, 1);  // start of segment
        events.emplace_back(r, -1); // end of segment
    }

    sort(events.begin(), events.end(), cmpEvent);

    int runningSum = 0, maxOverlap = 0, resPos = -1;

    for (Event e : events)
    {
        runningSum += e.delta;
        if (runningSum > maxOverlap)
        {
            maxOverlap = runningSum;
            resPos = e.pos;
        }
    }
    ...
    if (maxOverlap <= 1)
    {
        cout << "No common point found for more than one segment." << endl;
    }
    else
    {
        cout << "Position " << resPos << " is common to maximum segments: " << maxOverlap << endl;
    }
}
void HarHarMahadev()
{
