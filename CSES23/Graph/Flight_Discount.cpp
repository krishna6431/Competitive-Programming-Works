...
void HarHarMahadev()
{
    ...
    vector<vector<ll>> visited(n + 1, vector<ll>(2, 0));
    ...
    while (!pq.empty())
    {
        auto curr = pq.top();
        pq.pop();
        if (visited[curr.node][curr.take])
            continue;
        visited[curr.node][curr.take] = 1;

        for (auto x : graph[curr.node])
        {
            // if we have not taken discount earlier and we are taking discount now
            if (curr.take == 0)
            {
                if (dist[x.first][1] > curr.cost + x.second / 2)
                {
                    dist[x.first][1] = curr.cost + x.second / 2;
                    pq.push({dist[x.first][1], x.first, 1});
                }
            }

            // Just a Normal Case
            if (dist[x.first][curr.take] > curr.cost + x.second)
            {
                dist[x.first][curr.take] = curr.cost + x.second;
                pq.push({dist[x.first][curr.take], x.first, curr.take});
            }
        }
    }
