...
void HarHarMahadev()
{
    ...
    while (!q.empty())
    {
        ...
        for (int k = 0; k < 4; k++)
        {
            int newx = x + dx[k];
            int newy = y + dy[k];
            if (newx < 0 || newx >= n || newy < 0 || newy >= m || visited[newx][newy] || graph[newx][newy] == '#')
                continue;
            visited[newx][newy] = 1;
            path[newx][newy] = {x, y, direction[k]};
            q.push({newx, newy, x, y, direction[k]});
        }
    }
